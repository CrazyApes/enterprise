...
open class EmployeeServiceImpl(
	...
	override fun login(username: String, password: String): ResponseEntity<String> {
		...
		if (null == auth) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(gson.toJson("该用户名未注册"))
		} else if (auth.password != Security.encodeMD5(password)) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(gson.toJson("您输入的密码不正确"))
		} else {
